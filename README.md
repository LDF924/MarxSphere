...
<p align="center">
  ...
  <a href="https://github.com/LDF924/MarxSphere/actions"><img src="https://img.shields.io/badge/tests-1053%20passed-green" alt="Tests" /></a>
...
**AI 驱动的全人文社科学术科研平台** — 一个 AI Agent 承载科研与教学全能力：科研端（文献检索、知识图谱、52 步推理、科研场景工作台、实证分析）+ 教育端（个性化学习规划、作业辅导、学情诊断、教师备课）。
...
人文社科研究有独特的学术要求，通用 LLM 工具（ChatGPT/通用 Agent）难以满足：
...
6. **覆盖论文全生命周期**：选题打磨 → 文献调研 → 政策检索 → 实证分析 → 论文写作 → 评审,配套 208 个科研技能与全套工程纵深(成本可审计 / 多档路由 / 沙箱安全)——**为研究者提供一步到位的可验证工具链**

> **一句话**：MarxSphere 不是"又一个 AI 助手"，而是把人文社科研究的**证据链、方法论、学术规范**做成可运行、可审计、可复用的系统。

## 功能总览

> 📖 **完整功能规格**：见 [docs/FEATURES-DETAILED.md](docs/FEATURES-DETAILED.md)（52 步推理逐步表 / 66 场景清单 / 72 工具矩阵 / 17 实证功能(含元分析) / 桌面端细节 / 评测指标）
...
| 🤖 **AI Agent** | 72 工具自主调度（含 Notebook 图表模板/桌面控制）/ 5 层安全 / 5 层记忆 / 任务 DAG / 审批门 / 执行租约 |
...
| 📡 **IM 接入** | 飞书 / 钉钉 / Telegram / 企业微信 机器人远程对话（状态/项目/评测/审批/告警命令; 企业微信支持自建应用双向, 详见 [IM 接入](docs/IM-INTEGRATION.md)） |
...
> 全部 43 个视图截图见 [docs/assets/](docs/assets/)。
...
- **72 工具自主调度**：50 个 Agent 工具（检索/推理/实证/写作/代码/联网/图片/文件/**教育能力**/格式评测/论文质量）+ 22 个视图工具（政策库/知识页/文献库/图谱/任务/评测/告警等，43 视图能力全覆盖）——教育 Agent 对话中可直接调用 `education_service` 触发学习规划/辅导/诊断/备课等能力
...
**三库知识图谱**（`图谱` 视图）
- Graphiti：自建文献库蒸馏、21,337 实体、1,085 知识社区、11,702 超边关系
- Cognee：31,253 实体、248,417 关系、11,550 切片（Neo4j 11003）
- PG pgvector：文献切片向量（1024 维）+ 全文检索
...
政经 C 刊科研方法论：四步法选题 / 选题矩阵 / 悖论选题 / 概念命名 / 跨学科 / 模板检测 / 编辑校验 / 外审翻译 / 期刊匹配（80 本期刊库）

### 🎓 领域研究引擎（经典文本 / 学术研究 / 政经C刊 / AI+教育）

**经典文本研究**（S36-S40，`classical-text-service.ts`）——经典文本研究专用 5 大能力：
...
**政经 C 刊科研**（S66，`cjournal-service.ts`）——基于八篇 C 刊选题方法论整合：
...
**AI + 教育**（122 教育路由 + 32 学习引擎顶层，`education-service.ts` 等 19 服务）——教育 Agent 工作台（顶部「AI+教育」Tab，学生端/教师端双角色），深度联动 52 步推理链路：
...
- **教育反馈闭环**：学生/教师端右下角反馈浮标（赞/踩+备注）→ `edu_feedback` 表 → 满意率/负评热点统计 → 自动改进建议（低分指标 + 负评驱动）
- **教育评测 12 项指标**：技术 6 项（BKT AUC / 学情诊断 F1 / 路径无逆序 / 批改准确率 / 思政核验 / 闭环完成率）+ 教学效果 6 项（掌握度提升 / 辅导前后对照 / 备课效率 / 批改效率 / 规划覆盖率 / 用户满意度）——评测工作台可视化 + 自动改进建议
...
- **学术期刊库**：80 本期刊（南核 67 / C扩 9 / 北核 4），级别筛选 + 热点展开 + 一键填入选题四步法
...
LLM_API_KEY=sk-xxx            # 与下面的 LLM_BASE_URL 必须同源
LLM_BASE_URL=https://api.deepseek.com/v1
LLM_MODEL=deepseek-flash      # DeepSeek 端点只认 deepseek-flash / deepseek-v4-pro
...
本项目由邓富（LDF924）开发。开发过程中使用 **DeepSeek**（LLM 推理/代码生成）与 **Claude Code**（AI 编码代理）辅助编写、审查与调试代码。AI 生成的代码均已由开发者人工审查、测试与验证（1053 项单元测试全绿，53 题评测 0.884）。
