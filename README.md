...
马克思主义理论研究有独特的学术要求，通用 LLM 工具（ChatGPT/通用 Agent）难以满足：

| 痛点 | 通用工具的表现 |
|---|---|
| **原著原文溯源** | 回答凭记忆，不引原文，无法验证是否忠于《资本论》《矛盾论》等经典文本 |
| **二手资料幻觉** | 分不清"原著说了什么"与"别人说原著说了什么"，论证链条断裂 |
| **政策时效性** | 不了解最新中央文件、一号文件、全会公报，政策引用过时甚至错误 |
| **学术规范** | 不掌握 GB/T 7714 引文格式、公文 GB/T 9704 版式、C刊审稿标准 |
| **实证方法** | 无法做 DID/事件研究/元分析等规范计量，结论"看着像"但不可复现 |

### MarxSphere 的答案：为马理论学科构建的科研中枢

1. **原文可溯源**：501 篇原著文献入库（三库图谱），52 步推理每一步绑定 `chunk → event → entities` 证据链，回答可点击回看原文切片——**不编造，不靠记忆**
2. **证据优先**：引文三维核验（元数据真伪/语境相关性/断言支持度）、知识页 Compiled Truth 只写有证据支撑的结论——**反幻觉是架构纪律，不是提示词**
3. **学科全流程**：选题打磨（good-question）→ 文献调研 → 政策检索（gov.cn 库）→ 实证分析（17 方法含元分析）→ 公文/论文写作（GB/T 规范）→ 评审——**覆盖马理论科研的完整生命周期**
4. **教育与科研一体**：同一知识底座支撑个性化教学（BKT 掌握度/自适应路径/思政审核），科研成果直接转化为教学素材
...
> **一句话**：MarxSphere 不是"又一个 AI 助手"，而是把马克思主义理论研究的**证据链、方法论、学术规范**做成可运行、可审计、可复用的系统。
...
**AI + 教育**（121 教育路由 + 32 学习引擎顶层，`education-service.ts` 等 19 服务）——教育 Agent 工作台（顶部「AI+教育」Tab，学生端/教师端双角色），深度联动 52 步推理链路：
