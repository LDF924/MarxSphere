...
历经 0.1.0 → 0.16.8 共 100+ 次迭代修复，核心功能全部稳定：

- **桌面端完整装机流程**：引导页解压/下载/初始化全链路（进度条 + 雷达检测 + 一键修复/清理/返回引导页）
- **AI 对话与工具链**：服务商联动模型（DeepSeek/302AI）、动态模型识别、DB 设置生效、工具 49 个可调用
- **项目体系**：新建项目原地刷新、租户过滤修复、面板双向拉伸
- **数据库**：迁移等待/降级/一键修复、PG 下载多镜像加速
- **安全**：API key 清理、SSRF 防护、命令注入消除、评测/巡检需确认（防静默消费）
- **认证**：注册/登录/退出/忘记密码全链路
