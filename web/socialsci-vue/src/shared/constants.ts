/**
 * 共享常量与 localStorage key 总表 — 参考产品解码逐条收录(任务要求: 一个不漏)
...
// localStorage keys
export const K = {
  authToken: "skf_auth_token", // 全局 Bearer token(参考产品共享层 op; 我方回退 sag_token)
  settings: "skf_settings", // 全局设置(参考产品 settings store)
...
// window CustomEvent 事件名(参考产品 editor-review §4)
export const EVT = {
  ...
  // ⚠ 以下 6 个是**参考产品契约**, 不是死代码 —— 2026-09-12 审计时全项目零引用, 但参考产品文档
  //   明确它们的作用(见 .claude/reverse-engineering/socialsci-com/full/decoded-editor-review.md:83 与
  //   decoded-workflow-sections-input.md:47; 该目录为本地逆向归档, 不入库不同步,
  //   入口见 .claude/reverse-engineering/README.md 的「规格 ↔ 实现对照表」):
  ...
  //   即"参考产品有实现、我方尚未移植", 与真正没人听的死代码是两回事, 故保留待接。
  // review SSE 桥(参考产品 window 全局回调)
...
/** 时间短格式: <1h 分钟 / <1d 小时 / <7d 天 / 日期(参考产品 rail meta 语义) */
...
/** zh-CN 千分位 + MM-DD HH:mm(参考产品 SideBar meta 格式) */
