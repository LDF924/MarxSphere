...
export function PrismaPanel() {
  ...
  return (
      ...
      <div className="mt-2 grid grid-cols-2 gap-1.5 sm:grid-cols-4">
        {[
          { icon: "①", title: "输入主题", desc: "填主题检索文献库" },
          { icon: "②", title: "检索文献", desc: "关键词匹配, 记命中" },
          { icon: "③", title: "标题筛选", desc: "AI 判纳入/排除" },
          { icon: "④", title: "纳入集+摘要", desc: "统计+纳入综述集" },
        ].map((g) => (
          <div key={g.title} className="rounded-lg border border-emerald-400/15 bg-emerald-400/[0.03] p-1.5">
            <div className="text-[11px] font-medium text-emerald-200/90">{g.icon} {g.title}</div>
