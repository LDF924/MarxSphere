// universe-view-preferences.ts — 视图偏好持久化与归一化(对齐 Zleap universe-view-preferences.ts)
// 语义: localStorage 版本化 schema(v7);数字字段按 step 归整并夹持在
// UNIVERSE_VIEW_LIMITS;缓存容量下限由可见窗+双侧预取跑道导出;实体类别
// 过滤永不删除事件。来源: Zleap-AI/SAG apps/web/lib/universe-view-preferences.ts(完整移植)。
"use client";
...
export const DEFAULT_UNIVERSE_VIEW_PREFERENCES: UniverseViewPreferences = {
  ...
  cardsEnabled: true,
  eventCardPreviewCount: UNIVERSE_VIEW_LIMITS.eventCardPreviewCount.default,
  temporalPageSize: UNIVERSE_VIEW_LIMITS.temporalPageSize.default,
  temporalPrefetchPages: UNIVERSE_VIEW_LIMITS.temporalPrefetchPages.default,
  entityTypes: null,
  documentIds: null,
};
