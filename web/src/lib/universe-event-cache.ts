// universe-event-cache.ts — 事件 bundle FIFO 缓存(对齐 Zleap universe-event-cache.ts)
// 语义: 事实以事件 bundle 为单位驻留;同一事件键的后续内容合并(merge 取最新
// 载荷,origin 保留首次入场),容量超出时按准入顺序驱逐最旧键。
// 来源: Zleap-AI/SAG apps/web/lib/universe-event-cache.ts(完整移植)。
import type {
