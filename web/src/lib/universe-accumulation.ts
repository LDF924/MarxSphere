// universe-accumulation.ts — 累积呈现:多来源证据去重/排队/就地富化(对齐 Zleap universe-accumulation.ts)
// 语义: 重复事实进缓存合并并在原窗口位置就地富化,不改变窗口顺序;
// 视觉过渡活跃期的新证据进入 pending 队列,settle 后再补入。
// 来源: Zleap-AI/SAG apps/web/lib/universe-accumulation.ts(完整移植)。
import {
