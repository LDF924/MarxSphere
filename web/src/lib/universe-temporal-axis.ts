// universe-temporal-axis.ts — 源局部计数轴:事件包沿探索序的 3D 投影(对齐 Zleap universe-temporal-axis.ts)
// 语义: 深度是快照稳定的 ordinal(0=最新端),不是时钟时间——书里所有事件共享
// 同一时刻,时钟轴会塌缩成一点;计数轴让每个事件均匀消费同一段旅行距离。
// 视觉深度提示不在本模块:相机沿轴飞行时存在感由每帧相机位置决定。
// 来源: Zleap-AI/SAG apps/web/lib/universe-temporal-axis.ts(完整移植)。
/**
