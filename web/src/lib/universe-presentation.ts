// universe-presentation.ts — 呈现状态:节点涌现/细节闩锁/视觉变形(对齐 Zleap universe-presentation.ts)
// 语义: 一次可逆旅程进度 → 粒子→星→卡片 完整卡片始终是单一视觉对象;
// 细节闩锁在退出阈值前保持当前源,候选变化被忽略。
// 来源: Zleap-AI/SAG apps/web/lib/universe-presentation.ts(完整移植)。
