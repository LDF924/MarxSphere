// universe-temporal-flight.ts — 沿计数轴的时序飞行(对齐 Zleap universe-temporal-flight.ts)
// 语义: 纯状态——轮子样本加惯性,按钮滑翔到目标,共用同一组夹持;
// 相机刚体只应用深度*增量*,与 OrbitControls 旋转/平移/捏合自然组合。
// 来源: Zleap-AI/SAG apps/web/lib/universe-temporal-flight.ts(完整移植)。
/**
