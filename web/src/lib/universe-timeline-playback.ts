// universe-timeline-playback.ts — 自动播放计划(对齐 Zleap universe-timeline-playback.ts)
// 语义: 播放顺序只是呈现状态,驻留时间线始终保持规范的最新→最旧序;
// chronological 只从当前位置走向更新事件,绝不假装客户端拥有最旧事件。
// 来源: Zleap-AI/SAG apps/web/lib/universe-timeline-playback.ts(完整移植)。
/**
