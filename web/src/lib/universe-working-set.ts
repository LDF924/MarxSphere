// universe-working-set.ts — Explore 工作集:事件 bundle 原子准入/驱逐(对齐 Zleap universe-working-set.ts)
// 核心语义: 场景预算(UNIVERSE_SCENE_BUDGET)限制 Three.js 渲染量,驻留预算(UNIVERSE_RESIDENT_BUDGET)
// 限制内存常驻量;bundle 是准入/驱逐的最小单位,驱逐只在 bundle 边界发生,保护集与持久 pin 集
// 决定哪些内容不可驱逐。来源: Zleap-AI/SAG apps/web/lib/universe-working-set.ts(完整移植)。
import type {
  UniverseActivation,
