// universe-preview-plan.ts — 阅读卡片预览计划与视点卡片选择(对齐 Zleap universe-preview-plan.ts)
// 语义: 只约束 DOM 阅读卡片,场景窗口仍持有每个事件/实体/事实关系;
// 事件+实体是一个视觉知识包——事件获得阅读卡片后其相关实体都要带紧凑标签。
// 来源: Zleap-AI/SAG apps/web/lib/universe-preview-plan.ts(完整移植)。
