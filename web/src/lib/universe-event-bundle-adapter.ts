// universe-event-bundle-adapter.ts — 图/激活/时间线/补丁 → 事件 bundle 适配器(对齐 Zleap)
// 语义: 实体与关系只经由其限定的事件进入 bundle,过滤不会留下悬挂关系;
// 时间线页保留 ordinal,同一事件多来源合并时 origin 取首次入场。
// 来源: Zleap-AI/SAG apps/web/lib/universe-event-bundle-adapter.ts(完整移植)。
import type {
  SearchSourceHit,
  UniverseActivation,
