// universe-events.ts — Explore 事件总线:全局 CustomEvent 派发与视图状态(对齐 Zleap universe-events.ts)
// 语义: 激活/重置/聚焦/详情/询问/补丁/视图/呈现模式 全部经 window CustomEvent
// 广播;模块级 epoch/ask 序列保证时序;详情与询问目标可被 takePending* 取走。
// 注: 原版用 next-intl clientErrorMessage 生成询问提示,本地用中文字面量替代。
// 来源: Zleap-AI/SAG apps/web/lib/universe-events.ts(完整移植)。
import type {
  SearchResponse,
  UniverseActivation,
...
export function dispatchUniverseAsk(node: UniverseAskNode) {
  ...
  const detail: UniverseAskTarget = {
    ...
    prompt: node.kind === "entity"
      ? `围绕实体「${node.label}」继续检索相关证据`
      : `围绕事件「${node.label}」继续检索相关证据`,
  };
