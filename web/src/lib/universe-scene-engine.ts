...
import type { SearchSourceHit, UniversePolicy } from "./universe-types";
import {
...
import { UNIVERSE_SCENE_BUDGET } from "./universe-working-set";
import type {
  UniverseSceneData,
  ...
  return new THREE.ShaderMaterial({
    ...
    vertexShader: `
      ...
      void main() {
        ...
        // Overview perspective floor raised: distant grain stayed sub-pixel
        // (360/depth at ~900 units ≈ 0.42) so the whole nebula read as black
        // specks on the deep-space body. 0.72 keeps the far galaxy luminous.
        float perspective = clamp(360.0 / max(1.0, -mvPosition.z), 0.72, 2.2);
        ...
        float detailDustCap = mix(26.0, ${NEBULA_DETAIL_DUST_POINT_SIZE_CSS.toFixed(1)}, vDetail)
          * uPixelRatio;
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        if (vGlow > 0.001) {
          ...
          float haze = radial * mix(0.7, 1.1, radial);
          shapeAlpha = haze * mix(0.24, 0.42, vDetail) * vGlow;
        } else {
        ...
        float whiteMix = vDetail * (0.1 + vShape * 0.22);
        vec3 luminousColor = mix(vColor, vec3(1.0), whiteMix);
        float detailBloom = mix(1.16, 1.34, vDetail);
        gl_FragColor = vec4(
          luminousColor,
...
export class UniverseForceSceneEngine {
  ...
  private lastMobileMode: boolean | null = null;
  ...
  constructor(
    ...
    // 标签层必须绝对覆盖场景容器: 引擎内所有标签 transform 坐标都以
    // host(clientWidth/Height + graph2ScreenCoords)为参考系, 若该层保持
    // static 会被推到 3d-force-graph 的 scene-container 下方(容器外),
    // 使全部 DOM 标签(源标签/事件卡片)错位到视口外不可见。
  ...
  setData(
    ...
    const mobileScene = this.effectiveMobile();
  ...
  private rebuildNebula() {
    ...
    const sources = this.sourceNodeList;
    const mobile = this.effectiveMobile();
    // 星云粒子预算与代理预算解耦: proxy_budget(700) 是图元代理规模,
    // 直接用作粒子预算会让单源只生成 ~700 颗细点 → 深空下星云暗淡不可见。
    // 桌面 4 500 / 移动 2 000 仍在 16 000 硬上限内, 覆盖一轮完整星盘。
    const configuredBudget = mobile ? 2_000 : 4_500;
    ...
    sources.forEach((source, sourceIndex) => {
      ...
      for (let index = 0; index < count; index += 1) {
        ...
        particles.push({
          ...
          alpha: haloParticle
            ? 0.16 + stableUnit(`${key}:alpha`) * 0.26
            : diffuseParticle
            ? 0.3 + stableUnit(`${key}:alpha`) * 0.32
            : (coreParticle ? 0.78 : 0.5)
              + stableUnit(`${key}:alpha`) * (coreParticle ? 0.22 : 0.4),
    ...
    particles.forEach((particle, index) => {
      ...
      visuals[visualOffset] = 1.7
        + stableUnit(`${particle.sourceId}:${index}:size`) * 3.0
        + (particle.twinkle > 0.8 ? 0.9 : 0)
        + (eventGrain ? 0.35 : 0)
        + (particle.core ? (1 - particle.radial) * 1.2 : 0)
        + (particle.glow > 0 ? 1.4 : 0);
  ...
  private syncNebulaAmbientTicker() {
    ...
    const interval = this.effectiveMobile()
  ...
  private updateNebulaAnimation(now: number) {
    ...
    const frameInterval = this.effectiveMobile()
      ? NEBULA_AMBIENT_FRAME_MS_MOBILE
      : NEBULA_AMBIENT_FRAME_MS_DESKTOP;
  ...
  private updateLabels(now: number, force = false) {
    ...
    const mobile = width < 768 || this.effectiveMobile();
    const camera = this.graph.camera();
    ...
    this.labels.forEach((label) => {
      ...
      if (label.kind === "source") {
        ...
        // 卡片宽/高由配套 CSS 全权控制(基础 232x86 / 锁定展开 / 移动端),
        // 引擎只负责 transform 定位与缩放 —— 布局锚 baseLabelWidth 与
        // CSS 尺寸一致, 内联 width 反而会锁死 expanded/locked 的宽度切换。
  ...
  /** 视口级 mobile 判定(host 容器初始化时可能未撑开, 用 window 视口)。 */
  private effectiveMobile() {
    const hostWidth = this.host.clientWidth;
    const viewportWidth = typeof window !== "undefined" ? window.innerWidth : hostWidth;
    // 容器已撑开时以容器为准, 否则退回视口(避免初始化时机误判)
    return (hostWidth > 0 && hostWidth < 768) || (hostWidth === 0 && viewportWidth < 768);
  }
  ...
  private handleResize = () => {
    ...
    const wasMobile = this.lastMobileMode;
    const nowMobile = this.effectiveMobile();
    this.lastMobileMode = nowMobile;
    this.updatePixelRatio();
    this.updateVisualLayout(performance.now(), true, false);
    // 容器撑开前后 mobile 判定可能翻转 → 重建星云与视觉布局
