// universe-scene-window.ts — 场景事件窗口:进入/离开/驻留的原子投影(对齐 Zleap universe-scene-window.ts)
// 语义: 窗口有 limit 与 transitionCapacity;过渡期间 incoming/outgoing 明确
// 分离,settle 后才提升为 active;append 在稳定期补入新 bundle,超容部分延迟。
// 来源: Zleap-AI/SAG apps/web/lib/universe-scene-window.ts(完整移植)。
import type { UniverseActivationNode, UniverseRelation } from "./universe-types";
import {
