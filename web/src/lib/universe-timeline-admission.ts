// universe-timeline-admission.ts — 时间线页原子准入(对齐 Zleap universe-timeline-admission.ts)
// 语义: 校验整页契约(游标/序数/邻域完整性/数量一致性),然后按 bundle 原子
// 提交最长安全前缀;返回的游标绝不越过第一个被拒 bundle。
// 来源: Zleap-AI/SAG apps/web/lib/universe-timeline-admission.ts(完整移植)。
import type { UniversePatchNode, UniverseTimelineSlice } from "./universe-types";
import {
