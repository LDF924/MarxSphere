// universe-session-state.ts — 会话工作区状态机(对齐 Zleap universe-session-state.ts)
// 语义: home/exploration/accumulation 三态;搜索/助手/扩展只是证据来源,
// 都不隐式定义呈现模式;返回探索只在快照可用时恢复,否则回 home。
// 来源: Zleap-AI/SAG apps/web/lib/universe-session-state.ts(完整移植)。
import type { UniverseActivationOrigin } from "./universe-types";
