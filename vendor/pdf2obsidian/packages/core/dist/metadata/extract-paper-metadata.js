...
export function extractPaperMetadata(input) {
...
export function createPaperFrontmatter(metadata) {
...
function detectPaperMetadata(markdown, title) {
...
function createEmptyPaperMetadata(title) {
    return {
        title,
        authors: [],
        keywords: []
...
function detectYear(markdown, doi, arxivId) {
...
function getTitlePageMarkdown(markdown) {
    const body = stripFrontmatter(markdown);
...
function matchFirst(markdown, pattern) {
...
function splitCompactAuthorLine(line) {
    const tokens = normalizeNameTokens(line);
    if (tokens.length < 4) {
        return [];
    }
...
function normalizeNameTokens(line) {
...
function firstValidYear(value, currentYear) {
