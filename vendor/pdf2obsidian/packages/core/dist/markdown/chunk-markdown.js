 ...
 */
export function chunkMarkdown(markdown, chunkCharLimit) {
 ...
 * @returns 分割后的 Markdown 块数组
 */
function splitLargeSection(content, chunkCharLimit) {
 ...
 */
function mergeSmallChunks(chunks, chunkCharLimit) {
    const merged = [];
    let current;
...
function mergeHeadingPaths(left, right) {
    if (left.join('\n') === right.join('\n')) {
        return left;
    }
...
function splitBySoftLimit(content, chunkCharLimit) {
    const chunks = [];
 ...
 * @returns 分割后的 Markdown 块数组
 */
function findBestSplitPoint(content, chunkCharLimit) {
