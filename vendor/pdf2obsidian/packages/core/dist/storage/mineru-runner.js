...
export async function runMineru(input) {
...
async function runMineruLocal(input) {
    const env = {
        ...process.env
    };
...
async function runMineruOfficialApi(input) {
    ...
    const applied = await readOfficialJson(applyResponse, '申请 MinerU 官方上传 URL 失败');
...
async function readConfiguredToken(tokenEnv) {
...
async function runMineruHttpApi(input) {
    const baseUrl = await resolveApiUrl(input.apiUrl ?? '');
...
async function waitForMineruTask(baseUrl, taskId) {
...
async function waitForOfficialBatch(token, batchId) {
    ...
    while (Date.now() < timeoutAt) {
        ...
        const status = await readOfficialJson(statusResponse, '查询 MinerU 官方批量任务失败');
        if (status.code !== 0) {
...
async function readOfficialJson(response, failureMessage) {
...
async function extractOfficialZip(zipBytes, outputDir) {
...
async function writeImagesFromResponse(value, imageDir) {
...
function findFirstRecordByKey(value, keys) {
    if (!value || typeof value !== 'object') {
        return undefined;
    }
    if (Array.isArray(value)) {
...
export async function findPrimaryMarkdown(rootDir, preferredStem) {
...
function buildAlternativeUrl(originalUrl, newHost) {
    try {
...
async function resolveApiUrl(configuredUrl) {
