...
import { createTextGenerationClient } from '@pdf2obsidian/providers';
/**
 * 生成阅读材料
 * @param input 生成阅读材料的输入
 * @returns 生成阅读材料的结果
 */
export async function generateReadingAssets(input) {
...
function createReadingAssetSpecs(input, sourceContext) {
...
function trimMarkdownForReading(markdown, maxChars) {
...
function createCacheKey(input, spec) {
...
function validateReadingAsset(spec, content) {
    const warnings = [];
...
function countMeaningfulItems(content) {
    return content
        .split(/\r?\n/)
        .filter((line) => /^(\s*[-*]\s+|\s*\d+[.)]\s+|\|[^|]+.*\|$|#{2,6}\s+)/.test(line.trim()))
        .length;
}
function ensureBacklink(content, slug) {
