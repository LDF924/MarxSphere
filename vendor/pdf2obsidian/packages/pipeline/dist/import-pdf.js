...
import { translateMarkdownInChunks } from '@pdf2obsidian/core';
...
import { generateReadingAssets } from '@pdf2obsidian/core';
...
export async function importPdf(input) {
    const resume = input.completedSteps ?? new Set();
    // ── upload ────────────────────────────────────────────────────
    ...
    if (resume.has('obsidian_export')) {
        ...
        // 结构化参考文献：提取并创建桩笔记
        const structuredRefs = extractStructuredReferences([linkedOriginalMarkdown, translated.markdown].join('\n\n'));
        ...
        await notifyStep(input, 'obsidian_export', 'completed', stubResult.createdCount > 0
...
async function notifyStep(input, step, status, message) {
...
function formatNullableMetadataValue(value) {
    return value === undefined || value === '' ? '未获取' : value.toString();
}
function formatDetectedLanguage(language) {
    const labels = {
        'zh-CN': '中文',
        en: '英文',
        unknown: '未知'
...
function formatAutoLinkedReferences(links, discoveredReferenceAliasCount) {
    if (links.length === 0) {
...
function formatReadingAssetLinks(assets) {
    if (assets.length === 0) {
        return [];
    }
    const labels = {
