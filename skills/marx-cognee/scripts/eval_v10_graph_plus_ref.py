"""
Cognee RAG v10 — GRAPH_COMPLETION + qwen-plus + include_references=True
The missing combination: qwen-plus fidelity + deterministic Evidence blocks
"""
...
sys.path.insert(0, ".")
import dotenv
os.environ["LLM_MODEL"] = "openai/qwen-plus"
...
RESULTS = Path("eval/eval_results_v10_graph_plus_ref.json")
...
async def main():
    print("=" * 70)
    print("Cognee RAG v10 — GRAPH_COMPLETION + qwen-plus + include_references")
    print("=" * 70)
    ...
    # ── Phase 1: 收集 30 个答案 ──
    print(f"\nPhase 1: 收集 {len(gt_queries)} 个查询答案 (GRAPH_COMPLETION)...")
    collected = []
    ...
    print(f"\n{'='*70}")
    print(f"v10 评估完成 — GRAPH_COMPLETION + qwen-plus + include_references")
    print(f"{'='*70}")
