"""Step 2: 5-dim eval — GRAPH_COMPLETION + raw text augmentation + verify=ON (qwen3.7-max).
"""
...
OUTPUT = Path("eval/eval_results_step2_verify_on.json")
CHECKPOINT = Path("eval/.eval_checkpoint_step2.json")
...
async def main():
    print("=" * 70)
    print("Step 2: GRAPH_COMPLETION + raw text + verify=ON (qwen3.7-max)")
    print("=" * 70)
    ...
    if "collected" not in cp:
        print(f"\nPhase 1: Collecting {len(gt_queries)} answers (verify=ON, raw_text=ON)...")
        collected = []
    ...
    step1 = {}
    s1_path = Path("eval/eval_results_step1_rawtext.json")
    if s1_path.exists():
        step1 = json.loads(s1_path.read_text(encoding="utf-8")).get("summary", {})

    print(f"\n{'='*70}")
    print(f"RESULTS ({n} queries)")
    print(f"{'Dimension':15s} | {'Step1':>8s} | {'Step2':>8s} | {'Diff':>8s}")
    print("-" * 55)
    for d in dims:
        s1 = step1.get(d, 0)
        s2 = summary.get(d, 0)
        ...
        print(f"  {d:15s} | {s1:8.4f} | {s2:8.4f} | {sign}{delta:7.4f}")

    report = {"timestamp": time.strftime("%Y-%m-%dT%H:%M:%S"), "summary": summary, "step1_comparison": step1, "details": judged}
