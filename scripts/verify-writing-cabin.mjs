...
try {
  ...
  console.log("\n═══ ⑤ 对齐批次的结构断言(2026-09-15 B1-B4) ═══");
  {
    ...
    /**
     * ⚠ 2026-09-26 修: 原来是 `tileCount === 6`（手动添加 4 + 从其他模块导入 2）——
     *   又一个**写死数量**的断言。写作舱资料页新增第三个出口「数据分析台（17 种统计方法）」后
     *   变成 7, 这条就假失败了, 而页面本身完全正常。
     *   与上面 `groups` 那条注释记录的是**同一个教训**（"断言数量等于 N 会把正常的新增判成回归;
     *   断言'该有的在'才不会"）—— 当时只改了 groups, 把这条漏了。
     *   改判**结构**: 四个手动入口 + 三个模块出口都要在, 不锁总数。
     */
    const wantTiles = ["检索文献", "添加表格", "添加理论", "上传附件",
                       "前往数据分析模块进行分析", "前往科研绘图模块进行分析", "前往数据分析台（17 种统计方法）"];
    const missingTiles = r ? wantTiles.filter((w) => !r.tiles.includes(w)) : ["(面板未渲染)"];
    t("手动添加四格 + 从其他模块导入三格齐全", !!r && missingTiles.length === 0,
      r ? `缺: ${JSON.stringify(missingTiles)} · 实测 ${JSON.stringify(r.tiles)}` : "—");
    /**
