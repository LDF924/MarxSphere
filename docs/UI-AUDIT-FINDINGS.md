...
- 选题界定: 顶部大纲树编辑器 + 6字段组卡(主题/字数/框架/方法grid-3/文件拖拽/澄清QA) + 底部CTA
- 文献与资料: 工具条(智能生成/审视/编排3主按钮) + 手动添加/模块导入卡 + 素材清单(checkbox+目标章节select) + 2弹窗(计划确认)
- 框架设计: 状态卡(三态徽标+typewriter+Step x/3) + 框架概览(变量卡grid/假设ul/方法pills) + 章节写作指导卡列表 + 底部确认CTA
