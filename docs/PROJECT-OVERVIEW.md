...
一个 **AI Agent** 承载科研与教学全能力（72 工具统一调度 + 122 教育路由）：
...
- **教育能力（同一 Agent 延伸）**：个性化学习规划、作业辅导（分步骤提示/错因分析/同类题训练，不直接给答案）、学情诊断、教师备课（122 教育路由）——与科研共用同一推理与检索底座，教学与科研数据互通
...
5. **AI Agent 子系统**：60+ 能力项、72 通用工具（50 Agent + 22 视图）+ 教育专属 Agent（9 服务文件、122 教育路由）、自主研究、多 Agent 协作、记忆层
6. **实证研究工作台**：问卷生成/信效度/诊断/插补/回归/证据账本/质量闸门
7. **AI+教育工作台**：顶部「AI+教育」Tab（学生端「我的学习」/ 教师端「教师工作台」）——六大学习能力 + 苏格拉底式五步打磨（想法卡/追问/导出）+ 作业辅导闭环 + BKT 认知诊断 + 知识点先修图 + 思政内容审核 + 端到端自动闭环，122 教育路由
8. **桌面端**：Electron + NSIS 安装包，首次启动全量引导

### Agent 设计思路

- **决策循环**：规划 → 选工具 → 执行 → reflect → replan（最多 3 轮）
- **工具注册表**：72 通用工具（50 Agent + 22 视图）+ 教育工具集统一 schema 校验、白名单审批、超时熔断、重试退避
...
| 通道 | 默认模型 | 可替代 |
|---|---|---|
| LLM | deepseek-flash / qwen-plus | 任意 OpenAI 兼容 |
...
| 66 科研场景 | ✅ 完成（8 阶段全覆盖） |
| AI Agent | ✅ 完成（60+ 能力项，72 通用工具 + 122 教育路由，1080 测试） |
| 实证工作台 | ✅ 完成（10 功能实测通过） |
| AI+教育 | ✅ 完成（双端工作台，122 教育路由，教育评测综合分 0.884） |
| 桌面端 | ✅ 完成（NSIS 安装包） |
...
| 形态 | 说明 |
|---|---|
| **Web 工作台** | 48 视图 · Mega Menu 导航 · 7 大分类（对话推理/科研中心/知识中心/政策资料/技能工具/系统管理） |
