# MarxSphere 全链路调用图谱（2026-08-21 更新）

> 推理链路 + AI Agent + AI+教育 全链路调用关系。
...
│ ↓ │
│ trace → eval-32-metrics.ts │
│ → 31+ metrics → overall (A:0.40 B:0.35 C:0.25 D:0.00) │
└──────────────────────────────────────────────────────────────┘
```

## 评测链路 (eval-22-metrics.ts)
...
│ - A2 context_precision (逐个chunk, 连续分数) ★ │
│ - A3 context_relevancy (逐section) ★ │
...
| 问题分类 | | 4路分调 |
| 实体提取 | | NER阈值20, 短事实跳过 |
| Stage2 | | 三路并行 + embedding复用 + external_entities embedding |
| Stage3 | | callG 90s + DeepWalk 12-hop + entities/hybrid分离 |
| Stage4 | | 全局去重 + COT增量 + 空上下文guard + JSON容错 + 引用交叉检查 |
| **评测** | 当前 | 31+ 评分项（A9-A12 新增）+ 53 题金标集，基线 0.884 |
| 基础设施 | | MCP生命周期 + 分级错误 + DB重试 + PG超时 |


## AI+教育调用链路（84 路由）

```
用户 → 「AI+教育」Tab（学生端/教师端）
     → /api/education/*（84 路由）
     → 教育服务层（12 文件）：
        六能力(education-service) → 52步推理(deepReason) / 知识库检索(retrieveChunks)
        自适应四层(adaptive-learning) → PG(answer_history/knowledge_mastery)
        作业闭环(homework-help) → 错题/变式 → 掌握度联动
        五步打磨(agent-education) → polishStep(发散/验证/聚焦/压力) → 想法卡/追问
        BKT(cognitive-diagnosis) → p(掌握) 推断
        先修图(knowledge-graph-edu) → kp_edges 拓扑路径
        思政审核(content-audit) → Compiled Truth 校准
        自动闭环(auto-learning-loop) → 采集→诊断→迭代→周报
```
...
| 层 | 入口 | 关键依赖 |
|---|---|---|
| 推理 | `POST /api/reason/query` | 四源检索 + evaluate + reflection |
| Agent | `POST /api/agent/tasks` | 44 工具 + 审批门 + 记忆层 |
| 教育 | `POST /api/education/*` | 52 步推理 + source_chunks + PG 学情表 |
| 评测 | `eval-32-metrics.ts` / `eval-education.ts` | 金标集 + LLM Judge（双轨 0.884） |
