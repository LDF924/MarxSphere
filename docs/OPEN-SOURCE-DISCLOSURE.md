...
| 模型 | 类型 | 使用范围 | 原因 |
|---|---|---|---|
| deepseek-flash / qwen-plus | 专有 | 推理/实体抽取/综合回答 | 中文领域效果、成本、速度平衡 |
| qwen3-rerank | 专有 | 检索重排 | 与检索流水线配合的领域效果 |
| text-embedding-v4 | 专有 | 向量化 | 中文语义理解效果好 |
...
- **日志脱敏**：所有日志输出经 `sanitizeLine/sanitizeObject` 脱敏——**API 密钥、token、个人信息不进系统日志**（`src/services/log-sanitizer.ts`， 强制）
- **外部请求脱敏**：HTTP 层对外部请求错误信息脱敏，不暴露内部细节
...
**数据来源**（对齐 `src/services/education-compliance.ts` 实现）：

| 数据类型 | 来源 | 内容 | 授权与边界 |
|---|---|---|---|
| 学情数据（低敏） | 学生作答/练习/错题（`answer_history`、`learning_pace`、`knowledge_mastery`、`wrong_questions`） | 学科掌握度数值与作答记录，**默认匿名 student_id** | 仅用于个性化辅导与学情诊断，不采集个人标识 |
| 教学交互（中敏） | 学习计划/复盘/变式题（`study_plans`、`study_reviews`、`variant_questions`） | 按会话保存 | 可一键清理，不进评测语料 |
| 语音（高敏） | 口语测评/语音输入 | **不落库** | 仅本地处理 + 会话后即删，不训练；明示学生 + 提供「不使用语音」备选 |
| 教育复用资产 | 场景模板/教学案例库/示例课程（`education-templates/`、`education-cases.json`） | 团队自建演示资产 | 开源随代码分发，按角色空间隔离（学生/教师独立） |
| 模拟评测数据 | `data/edu-sim-student*.json`（确定性种子） | 匿名学生作答序列 | 随机模拟，无真实个人信息，仅用于评测/演示 |
| 使用反馈（低敏） | 学生/教师反馈（`edu_feedback` 表，迁移 086） | 赞/踩 + 备注 + 场景，**匿名 student_id + sanitizeLine 脱敏** | 仅用于教学效果统计与改进驱动（满意度/负评热点），不采集个人标识，可一键清理 |

**脱敏方式**：
- **日志脱敏**：教育服务日志一律经 `sanitizeLine` 处理（`src/services/log-sanitizer.ts`）——student_id、token、语音内容不进日志
- **匿名化**：默认匿名 student_id，不采集姓名/学号/手机号等个人标识
- **最小化采集**：仅采集学习过程数据（作答/错题/计划/复盘），不采集个人信息、录音录像、生物特征
- **输出脱敏**：评测/报告输出不含学生标识，班级学情为聚合数据

**隐私保护与生命周期**：
- **数据分级**：低敏（学习行为）/ 中敏（教学交互）/ 高敏（语音，仅本地+即删）三级，对应不同保护策略
- **权限隔离**：教育路由经 `/api/education` 权限门（ALLOWED_PERMS 校验）——无 `education` 权限的调用一律拒绝（测试覆盖）
- **保留期**：学情数据默认保留 **30 天**（`EDU_DATA_RETENTION_DAYS` 可覆盖），超期自动清理（`cleanupStudentData` + 定期巡检）
- **一键清理**：学生/教师可在面板一键删除某学生的全部学情数据（作答/错题/掌握度/计划/复盘）
- **语音即删**：语音数据会话后立即删除，不落库不训练
- **知情与备选**：口语测评等敏感功能明示学生，提供「不使用语音」的完全备选路径
- **家长/教师知情流程**：真实课堂试点采集前需家长/教师知情同意（计划见 [EDU-EVALUATION.md §2.3](EDU-EVALUATION.md)）
- **合规自证**：`/api/education/compliance/status` 提供数据分级表 + 采集范围声明 + 清理操作记录 + 状态查询

### 11.5 上下文、记忆与状态管理（Agent）
