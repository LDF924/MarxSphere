...
export interface RerankClient {
  ...
  /** 带连续分重排（rerank_score — GBrain rerank_score 字段） */
  rerankEventsWithScores(input: {
    query: string;
    candidates: EventRecord[];
    topK: number;
...
export class QwenRerankClient implements RerankClient {
  async rerankEvents(input: {
    ...
    const scored = await this.rerankEventsWithScores(input);
    return scored.map((entry) => entry.id);
  }

  async rerankEventsWithScores(input: {
    query: string;
    candidates: EventRecord[];
    topK: number;
  }): Promise<Array<{ id: string; score: number }>> {
    ...
    if (!settings.hasRemoteLlm) {
      ...
      return ids.map((id) => ({ id, score: 0.5 }));
    }
  ...
  private async remoteRerank(settings: AiRuntimeSettings, input: {
    ...
    const baseUrl = config.RERANK_BASE_URL ?? settings.llmBaseUrl;
    const dashScope = isDashScopeHost(baseUrl);
    ...
    const topN = Math.min(input.topK, input.candidates.length);
    const body = dashScope
      ? {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      const log = createModelCallLogger({
        kind: "llm",
        operation: dashScope ? "rerankEvents.dashScopeRerank" : "rerankEvents.qwen3Rerank",
        request: {
          url,
          method: "POST",
          attempt,
      ...
      try {
        const response = await fetch(url, {
          ...
          headers: {
            // rerank 走 DashScope 原生端点时用阿里云 key（embedding 同账户，非 LLM key——LLM 已切 deepseek）
            Authorization: `Bearer ${dashScope ? settings.embeddingApiKey : settings.llmApiKey}`,
            "Content-Type": "application/json"
          },
        ...
        const scored = parseRerankScored(responseBody, input.candidates, input.topK);
        ...
        return scored.length > 0
          ? scored
          : localScoreRerank(input.query, input.candidates, input.topK).map((id) => ({ id, score: 0.5 }));
      } catch (error) {
        lastError = error;
        if (!logged) {
          log.fail(error);
...
function isDashScopeHost(baseUrl: string): boolean {
  try {
    return new URL(baseUrl).hostname === "dashscope.aliyuncs.com";
  } catch {
    return false;
  }
}
...
function parseRerankResponse(responseBody: unknown, candidates: EventRecord[], topK: number): string[] {
  return parseRerankScored(responseBody, candidates, topK).map((entry) => entry.id);
}

/** 带连续分解析（rerank_score 支持 — GBrain rerank_score 字段） */
function parseRerankScored(responseBody: unknown, candidates: EventRecord[], topK: number): Array<{ id: string; score: number }> {
