 ...
 *
 * ⚠⚠ **当前状态: 引文抓不到数据(2026-09-19 实测)**。原因与我先前的判断**不同**, 记准:
 *
 *   **不是**"知网改版 / 结构变了" —— 逐篇实测**结构完好**:
 *     4 篇期刊论文的 `#refpartdiv li` **都是 6 个**, class 就是我们依赖的
 *     `references / citations / coreferences / cocitations / secondreferences / secondcitations`。
 *     我先前说"结构已不存在"是**错的**(那次是被验证码页挡了, 两个原因表现一样, 我没分辨)。
 *
 *   **是"知网没有把引文数据发到这个页面"**。同一篇期刊论文上实测:
 *     · 全页 `[N]` 样式的行 = **0**; 页面全文只有 **2066 字符**;
 *     · 自己的引文容器全空: `rc1`~`rc6` = 0 字节, `nxgp-kcms-data-ref-*` = 0 字节;
 *     · **点「参考文献」tab 期间只发出两条埋点上报**(`support.cnki.net/applog/front`),
 *       **没有任何数据请求** —— 说明页面内渲染这条路当前拿不到数据。
 *
 *   ⇒ 本模块按 `#refpartdiv` 取数是**对的**(结构没变), 但**没有数据可取**。
 *     要修得先搞清知网现在从哪儿给引文(登录态/机构权限/新入口), 那是新的调研。
 *
 *   消费者: 目前**只有 `web/src/lib/api.ts` 的一层封装, 没有任何 UI 调用** ——
 *     也就是它既不工作、也没人用。**在拿到数据之前不要接进界面**,
 *     否则用户看到的就是"这个功能坏了"。
 *
 *   仍然可靠的用途: `searchAndOpen`(检索并打开详情页)与 `readIdentity`(读登录身份) —— 实测可用。
 *
 * 原设计(仍有效, 只是取不到数据): 通过 CDP proxy(localhost:3456)驱动浏览器知网页面:
 * 1. 找到知网详情页 tab
...
export async function fetchCnkiCitations(
  ...
  if (!markResult.startsWith("ok")) {
    /**
     * ⚠ 这句此前写的是"页面上未找到引文 tab", 会把人带偏 —— 让人以为页面没这个功能区。
     *   而实测(2026-09-19) 6 个 tab **都在**, class 也对; 真实情况是**没有数据可取**(详见文件头)。
     */
    return { ok: false, type, items: [], error: "知网未向页面下发引文数据(6 个引文 tab 在, 但点它们不触发取数) —— 引文抓取当前不可用, 详见文件头", tabFound: false };
  }
  ...
  if (items.length === 0) {
    return {
      ...
      error: "未解析到引文条目 —— 实测该页面根本没有下发引文数据(疑似需特定登录态或入口), 也可能是该文献确无此类引文(如报纸/资讯类)"
    };
  }
