...
## 评测体系 (eval-22-metrics V3)

完整的 RAGAS v3 评测体系, 27 个观测指标覆盖检索质量(A)/答案质量(B)/推理质量(C)/效率(D)四个维度。

- **评测脚本**: `scripts/eval-22-metrics.ts` (822行) — 5种评测方法 (规则/LLM单评/LLM批量/LLM数组/Embedding语义)
- **评测标准**: `SCORING_STANDARD.md` — 27指标全量定义, 维度权重, 公式说明
- **架构文档**: `ARCHITECTURE.md` — 推理链路 + 评测链路完整架构
- **调用图谱**: `SAG_PIPELINE_CALLGRAPH.md` — LLM Judge / Embedding 调用链路
- **踩坑记录**: `PITFALLS_V8_V9.md` — V3版15项缺陷审计

综合分公式: `0.40×A + 0.35×B + 0.25×C` (D维度纯观测不计入)

```bash
# 运行评测
cd SAG-main && rm -f eval_32metrics.json && npx tsx scripts/eval-22-metrics.ts
```
...
```bash
bash scripts/sag-healthcheck.sh
```
